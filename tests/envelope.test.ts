...
test('registration requires distinct valid signatures over the exact contract and party list', async () => {
  const base = fresh();
  assert.equal(await canRegister(base), false);
  const one = await signed(base, alice);
  assert.equal(await canRegister(one), false);
  const both = await signed(one, bob);
  assert.equal(await canRegister(both), true);
  assert.equal(
    await canRegister({ ...both, source: base.source.replace('42', '43') }),
    false,
  );
  assert.equal(
    await canRegister({ ...both, parties: [nativeParty(alice)] }),
    false,
  );
  assert.equal(
    await canRegister({
  ...
  assert.equal(await canRegister({ ...base, parties: [] }), false);
  await assert.rejects(registrationPayload(one, 'tractate_v1'));
  assert.match(
    (await registrationPayload(both, 'tractate_v1')).data_item,
    /^[a-f0-9]{64}$/,
  );
});
test('share link round-trips values and signatures and ignores forged attestations', async () => {
  ...
  assert.equal(await canRegister(restored), true);
  const forged = {
    ...envelope.signatures[0],
    signature: '00'.repeat(64),
  };
  const noisy = { ...envelope, signatures: [forged, ...envelope.signatures] };
  assert.equal((await verifiedSigners(noisy)).length, 2);
  assert.deepEqual(
    await registrationPayload(noisy, 'tractate_v1'),
    await registrationPayload(envelope, 'tractate_v1'),
  );
