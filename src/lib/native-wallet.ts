...
import { nativeParty, signEnvelope, type Envelope } from './envelope';
...
const storageKey = 'tractate:native-wallets:v1';
...
export function readWallets(): Keystore[] {
  const stored = JSON.parse(localStorage.getItem(storageKey) || '[]');
  if (!Array.isArray(stored) || stored.length > 20)
    throw new Error(
      'Cannot read local wallets. Import a wallet backup to recover.',
    );
  return stored.map(parseKeystore);
}
export function storeWallet(wallet: Keystore) {
  const wallets = readWallets();
  if (!wallets.some((item) => item.publicKey === wallet.publicKey))
    wallets.push(parseKeystore(wallet));
  if (wallets.length > 20)
    throw new Error('This browser already has 20 wallets.');
