...
export function approvedReference(reference: string) {
  ...
  const record = approved.find((entry) => entry.reference === reference);
  if (!record)
    throw new Error(
...
export async function loadTemplate(reference: string, fresh = false) {
  ...
  if (fresh || !cache.has(reference)) {
    const request = (async () => {
      ...
      const { compileDocument } = await import('./compile');
...
export async function bindTemplate(
  ...
  const candidates = reference ? [approvedReference(reference)] : approved;
